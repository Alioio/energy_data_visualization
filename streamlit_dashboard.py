...
def create_chart(summary,  aggregation='mean', seperation_var='priceGuaranteeNormalized', seperation_value=12, date_interval=['2022-07-17', '2022-10-17'], widtht=700, height=280,selected_variable='dataunit', events_df=None):
    ...
    base = alt.Chart(source).mark_line(size=3).encode(
        ...
        y = alt.Y(aggregation+':Q', axis = alt.Axis(title=y_axis_title, offset= 5, format='ct/kWh')),
    ...
    chart = base.encode(
        ...
        x=alt.X('date:T',axis= alt.Axis(grid=False, title=''), scale=alt.Scale(domain=interval.ref())),
        y=alt.Y(aggregation+':Q', axis = alt.Axis(title=y_axis_title),scale=alt.Scale(domain=list(domain2),  offset= 5, format='ct/kWh')),
...
for i, energy_selection in enumerate(energy_type_selections):
    if((energy_selection == 'Strom')):
        ...
        chart_header = "**Entwicklung der Gaspreise**"
