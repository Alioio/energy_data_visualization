...
def read_energy_data(energy_type, verbrauch):
    ...
    wa_df = pd.read_excel('D:\energy_data_visualization\data\wa_{energy_type}.xlsx'.format(energy_type=energy_type))
...
#summary = summarize(high_consume, seperation_var,int(selection_slider))
    ...
    if(len(date_interval) == 2):
        energy_line_chart_e = create_chart(summary,mean_median_btn, int(selection_slider), date_interval=date_interval)
